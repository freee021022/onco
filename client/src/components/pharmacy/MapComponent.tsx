...
const MapComponent = ({ pharmacies }: MapComponentProps) => {
  ...
  // Otteniamo la chiave API di Google Maps dal server
  const { data: configData, isLoading } = useQuery<{ apiKey: string }>({
    queryKey: ['/api/config/google-maps'],
    staleTime: Infinity, // La chiave non cambierà durante la sessione
  });
    ...
    return (
      <div className="w-full h-full flex items-center justify-center bg-neutral-100">
        <div className="text-center p-4">
          ...
          <p className="mt-2 text-neutral-600">{t('pharmacy.loadingMap')}</p>
        </div>
      </div>
    );
  }
      ...
      {configData?.apiKey ? (
        ...
        <div className="w-full h-full flex items-center justify-center bg-neutral-100">
          <div className="text-center p-4">
            ...
            <p className="text-neutral-600">Impossibile caricare la chiave API di Google Maps</p>
          </div>
        </div>
      )}
